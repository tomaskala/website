...
date: 2026-07-09T20:45:28+02:00
---

The second set of the [cryptopals](https://cryptopals.com/) challenges focuses on block ciphers. Everything is encrypted using AES, but the cipher itself isn't important - all attacks take advantage of a weak mode of operation. The lesson here is that no matter how secure your underlying block cipher is, you can still introduce catastrophic security holes by messing up the way the blocks are combined together.

Most of the tasks focus on thoroughly breaking the ECB mode, which is the simplest mode of operation imaginable (discussed [before](/posts/cryptopals-set-1)). Towards the end, we also start breaking the CBC mode, which we will then continue in Set 3.
...
The decryption process works like this: ![CBC-decryption](CBC_decryption.svg)
Compared to encryption, decryption can run in parallel because we only need the ciphertext blocks, not the plaintext.
...
One more preparation task before we start breaking crypto. Here we implement an oracle that takes our input, prepends a random prefix and appends a random suffix. It then flips a coin and randomly encrypts the result in either ECB or CBC mode. Our task is to detect which mode was used by only looking at the ciphertext.
...
We implement an oracle that accepts a plaintext, appends an unknown string to it, encrypt the result under the ECB mode, and returns the resulting ciphertext. It turns out that we can recover the unknown suffix by repeated calls to the oracle with suitably crafted input, without knowing the encryption key. It works like this:
...
2. Build a mapping between encrypted blocks and plaintext bytes.
    ...
    Knowing the block size, we can now create a bytestring filled with 'A' that's exactly one letter shorter than the block size. If we feed it to the oracle, it will append the unknown suffix, making its first byte appear in the last position of the block. It will then return the corresponding ciphertext. We can determine this first byte by a simple dictionary lookup.

    We build a dictionary that looks like this:
    ```
...
3. Recover the unknown suffix one byte at a time.
    ...
    Rather than me attempting to explain this, it might be helpful to see what happens in four interesting scenarios. To make it shorter, we will assume block size of 8 bytes. The unknown suffix is `UNKNOWNSUFFIX`, with padding shown as `P`. The question mark `?` symbolizes the byte we iterate when learning the dictionary above.
    ...
    3. Decrypting the eighth byte (the last in the first block)
    ```
    learning the mapping:
    UNKNOWN?
    SUFFIXPP
    ...
    ```
    key = oracle(bytestring[N-block-size:N])[0:block-size]

    where N = length(bytestring)
          bytestring = (<padding> || <decrypted part of the suffix> || b)
...
We are given an API that, given an email address, returns an encrypted URL-encoded cookie that authenticates a user with that email as a regular user. The API itself is well-written and properly escapes all special characters, so we can't just inject arbitrary input. Still, because the cookie is encrypted under the ECB mode, we can edit it to instead authenticate as the admin. That is, change
```
...
I decided to implement the URL encoding properly using Go's [net/url.Values](https://pkg.go.dev/net/url@go1.26.5#Values) type. This is the correct thing to do, but it complicated the task, because its ordering is non-deterministic (the `Value` type is an alias for `map[string][]string`). I decided to cover two cases - one where the encoding follows the ordering given in the task (`email=foo@bar.com&uid=10&role=user`) and another that I actually observed (`email=foo@bar.com&role=user&uid=10`). The other permutations wouldn't be difficult to cover if needed.

The attack again takes advantage of the ECB mode encrypting the same blocks to the same ciphertext. This time, we will craft a suitable email input and cut the resulting ciphertext into blocks. We will then rearrange them and obtain a ciphertext representing an admin cookie. I didn't bother with the email input being a valid address, but it would be trivial to change this.

1. Ordering `email=foo@bar.com&uid=10&role=user`

We need to create one block that begins with `admin` and another block that ends with `role=`. We will then use them to assemble an admin cookie. The `admin` and `role=` strings must be at the block boundaries, because we cannot just slice into the middle of a block - that would scramble the contents.

The following two email queries return the cookies displayed below (divided into blocks for readability):
```
...
The assembled admin cookie is this:
```
...
This would be really easy if we happened to know the length of `random-prefix` and it was a multiple of the block size. We could simply call our attack from Challenge 12 with the following oracle function:
```
oracle(input-string) = prefix-suffix-oracle(input-string)[prefix-length:]
```
The prefix length being a multiple of the block size is important; otherwise slicing it away wouldn't work. That's because the bytes exceeding the block size would get encrypted into the same block as the first part of the input string, making it unreadable.
...
2. The random prefix contains no neighboring repeated blocks. If it did, we could instead detect the repetition starting from the end of the ciphertext. If even the unknown suffix repeated blocks, we would just collect all repetition starts and try them one by one.

# [Challenge 15](https://cryptopals.com/sets/2/challenges/15)

This challenge simply has us implement a PKCS#7 unpad function, which I already did in Challenge 09. The function must signal when the padding is invalid; this is something we will use later in Set 3 to implement a CBC padding oracle attack.
...
```
we have to edit `ciphertext(input)` so that it contains `;admin=true;`.

Coming back to the CBC decryption diagram, we see that each block of ciphertext is XORed with the next decrypted block to form the plaintext block: ![CBC-decryption](CBC_decryption.svg)
If we edit a position in the ciphertext block, it will completely scramble the corresponding plaintext block, but the same edit will propagate to the _following_ plaintext block. We can use that.
...
edited-block[0]  = edited-block[0]  XOR input[0]  XOR ';'  # 0 is the position of the first A
edited-block[6]  = edited-block[6]  XOR input[6]  XOR '='  # 6 is the position of the second A
edited-block[11] = edited-block[11] XOR input[11] XOR ';'  # 11 is the position of the third A

new-ciphertext = ciphertext[0:16] + edited-block + ciphertext[32:length(new-ciphertext)]
```
(We again assume a known block size of 16 bytes).
